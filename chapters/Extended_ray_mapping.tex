...
\If {$\lineaj\neq \nline$}
\State\Return{\Call{Intensity computation}{$\variabile{q}^{\,\textrm{a}},$  $\variabile{q}^{\,\textrm{c}},$ $\variabile{q}^{\,\textrm{a}}_{\lineaj},$ $\variabile{q}^{\,\textrm{c}}_{\lineaj},$ $\dir{}{}$, $\variabile{p}^{\textrm{a}}_{\lineaj}$, $\variabile{p}^{\textrm{c}}_{\lineaj}$, $\Pi_{\textrm{a}}$, $\lineaj$}}
\EndIf 
\State\Return{\Call{Intensity computation}{$\variabile{q}^{\,\textrm{d}},$  $\variabile{q}^{\,\textrm{b}},$ $\variabile{q}^{\,\textrm{d}}_{\lineaj},$ $\variabile{q}^{\,\textrm{b}}_{\lineaj},$ $\dir{}{}$, $\variabile{p}^{\textrm{d}}_{\lineaj}$, $\variabile{p}^{\textrm{b}}_{\lineaj}$, $\Pi_{\textrm{d}}$, $\lineaj$}}
\EndIf
\State \EndProcedure
\end{algorithmic}
\end{algorithm}
...
$10^7$     & $1.15 \cdot 10^{-6}$  & $2491.32$  \\
 ...
 \end{table}
\section{Conclusions}
In this chapter we present the extension of the analytic ray mapping method to system formed by curved lines. Employing the inverse ray tracing and a bisection procedure in target PS, an inverse map from the target to the source is constructed such that all the possible paths that the rays can follow are determined. The extended ray mapping method is able to detect the rays located on the boundaries of the regions with positive luminance. From these rays the target intensity is calculated. 
\\ \indent
We shown the results for two optical systems: the TIR-collimator and a parabolic reflector. To obtain a good accuracy of the boundaries in target PS  we divided the target into bins we apply the bisection method to each bin. A stopping criterion based on \'{e}tendue conservation is developed to determine the number of bins needed to obtain a good accuracy. Furthermore for the parabolic reflector many paths can occur along the reflectors. Etendue conservation is used again to determine the number of multiple reflections to be considered. The results show that the method is able to detect all the possible paths and only the rays on the boundaries rae traced through the system. Comparing our method with QMC ray tracing, significant advantages are observed in the accuracy and the computational time.
% Explain the results
