...
% Why the boundaries are important
Ries and Rabl (1994) showed that the boundaries 
$\partial$\set{R}{$1$}{}$(\Pi)$ at the source are mapped into the boundaries $\partial$\set{R}{}{}$(\Pi)$ at the target: all rays that are neighbors at the source PS remain close to each other at the target PS, \cite{Ries:2}.
...
three more rays with coordinates $(x_k, \tau_k)_{k = 5, 6,7}$ are traced. Each refinement step leads to four new triangles (see Figure \ref{fig:refinement}).
 \begin{figure}[htp]
  \begin{center}
  \includegraphics[width=5.7cm]{Error_cup}
  \end{center}
...
\begin{figure}[t]
  \begin{center}
  \includegraphics[width=5.5cm]{Error_cup}
  \end{center}
