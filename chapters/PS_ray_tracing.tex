...
Given a path $\Pi$, the map $\map{M}{}{}(\Pi)$: \set{R}{$1$}{}$(\Pi)$ $\mapsto$ \set{R}{}{}$(\Pi)$
...
$\partial$\set{R}{}{}$(\Pi)$ for all the possible paths $\Pi$. Indicating with $\variabile{q}^\textrm{min}(\Pi,\variabile{p})$ and $\variabile{q}^\textrm{max}(\Pi,\variabile{p})$ the minimum and the maximum position coordinates of the rays located on the boundary \set{R}{}{}$(\Pi)$ and using Eq. (\ref{eq:PSluminance}), Eq. (\ref{eq:PSintensity}) reduces to:
...
The aim is to construct a ray tracing procedure that allow us tracing more rays close to the discontinuity of the luminance, i.e. close to the boundaries $\partial$\set{R}{}{}$(\Pi)$.
To this purpose, a triangulation refinement at \set{S}{}{} is defined as explained in the following. \\ \indent
The regions \set{R}{}{}$(\Pi)$ can be defined only when some rays are traced.
...
\begin{figure}[h]
  ...
  The algorithm is not able to detect that region and, a further refinement is required.}
    %The parameters values are $\varepsilon_{variabile_{max}}~=~ 2$, $\varepsilon_{\tau_{max}}= 1$, $\varepsilon_{x_{min}}= 4$ and $\varepsilon_{\tau_{min}}=2$. }
...
%The distribution of the rays at the target is shown in Figure
...
\State Trace the ray with initial coordinatees $(\variabile{q}_1^\variabile{k}, \variabile{p}_1^{\variabile{k}})$ in \set{S}{}{};
%\State $pcoordinates\gets [p_coordinates \variabile{q}_1^\variabile{k}]$;
...
\State Left triangle(VL, $\varepsilon_{\variabile{q}_1}^{\textrm{min}}, \varepsilon_{\variabile{q}_1}^{\textrm{max}}, \varepsilon_{\variabile{p}_1}^{\textrm{min}}, \varepsilon_{\variabile{p}_1}^{\textrm{max}}$) \Comment{Refine the left triangle} \\
\State Right triangle(VR, $\varepsilon_{\variabile{q}_1}^{\textrm{min}}, \varepsilon_{\variabile{q}_1}^{\textrm{max}}, \varepsilon_{\variabile{p}_1}^{\textrm{min}}, \varepsilon_{\variabile{p}_1}^{\textrm{max}}$) \Comment{Refine the right triangle} \\
%\EndProcedure
\end{algorithmic}
\end{algorithm}
\begin{algorithm}
...
\begin{algorithmic}[1]
\Procedure{ Left triangle}{VL, $\varepsilon_{\variabile{q}_1}^{\textrm{min}}, \varepsilon_{\variabile{q}_1}^{\textrm{max}}, \varepsilon_{\variabile{p}_1}^{\textrm{min}}, \varepsilon_{\variabile{p}_1}^{\textrm{max}}$}
...
\If{$\textrm{dist}_\variabile{q}>\varepsilon_{\variabile{q}_1}^{\textrm{max}} $ or $\textrm{dist}_\variabile{p} >\variabile{p}_1^{\textrm{max}}$}
...
\EndIf
\EndFor
\If{$\textrm{dist}_\variabile{q}>\varepsilon_{\variabile{q}_1}^{\textrm{min}}$ or $\textrm{dist}_\variabile{p}>\varepsilon_{\variabile{p}_1}^{\textrm{min}}$}
\State RefineTriangle $\gets$  DifferentPath;
\Else
\If{(DifferentPath is true)}
\State Boundary(\textrm{VL}) $\gets$ true; \Comment{A boundary crosses the triangle}
\EndIf
\EndIf
...
\EndIf
\EndFor
\State\Return{\Call{Left Triangle}{$[\textrm{VL}(1),5, 7], \varepsilon_{\variabile{q}_1}^{\textrm{min}}, \varepsilon_{\variabile{q}_1}^{\textrm{max}}, \varepsilon_{\variabile{p}_1}^{\textrm{min}}, \varepsilon_{\variabile{p}_1}^{\textrm{max}}$}}
\State\Return{\Call{Left Triangle}{$[5,\textrm{VL}(2), 6], \varepsilon_{\variabile{q}_1}^{\textrm{min}}, \varepsilon_{\variabile{q}_1}^{\textrm{max}}, \varepsilon_{\variabile{p}_1}^{\textrm{min}}, \varepsilon_{\variabile{p}_1}^{\textrm{max}}$}}
\State\Return{\Call{Left Triangle}{$[7,6,\textrm{VL}(3)], \varepsilon_{\variabile{q}_1}^{\textrm{min}}, \varepsilon_{\variabile{q}_1}^{\textrm{max}}, \varepsilon_{\variabile{p}_1}^{\textrm{min}}, \varepsilon_{\variabile{p}_1}^{\textrm{max}}$}}
\State\Return{\Call{Right Triangle}{$[5,6, 7], \varepsilon_{\variabile{q}_1}^{\textrm{min}}, \varepsilon_{\variabile{q}_1}^{\textrm{max}}, \varepsilon_{\variabile{p}_1}^{\textrm{min}}, \varepsilon_{\variabile{p}_1}^{\textrm{max}}$}}
\EndIf
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\begin{figure}[h]
  \begin{center}
  \includegraphics[width=9cm]{triangulation_source}
  \end{center}
...
\\ \indent
The triangulation refinement provides more triangles close to the boundaries $\partial$\set{R}{$1$}{}$(\Pi)$ and only few triangles inside the regions \set{R}{$1$}{}$(\Pi)$.
All the possible paths $(\Pi)_{\variabile{j} = 1, \cdots, \npath}$ with are found and their corresponding regions \set{R}{$1$}{}$(\Pi)_{\variabile{j} = 1, \cdots, \npath}$ are determined. Using the edge-ray principle, we conclude that also the regions \set{R}{}{}$(\Pi)_{\variabile{j} = 1, \cdots, \npath}$ at the target are determined and only the rays close to the boundaries
$\partial$\set{R}{$1$}{} need to be considered to obtain the target rays distribution.
\section{Conclusions}
...
The boundaries $\partial$\set{R}{}{}$(\Pi)$ are particular relevant because there occur the discontinuities of the luminance. In particular, 
assuming a lambertian source, the rays on the boundaries gives all the information required to compute the target intensity. Based on this idea, a triangulation in \set{S}{}{} is construct such that the rays closest to the boundaries $\partial$\set{R}{}{}$(\Pi)$
...
Since Sobol's sequences are defined in a unit square, we scaled it such that all the source PS \set{S}{}{}$=[-2, 2]\times[-1, 1]$ is covered by rays. QMC ray tracing it considers as initial set, rays distributed as the points of a Sobol sequence. Such regular distribution can leads to several advantages for the computation of the target intensity, see Section \ref{sec:QMC} of this thesis. Finally, Figure \ref{fig:ps_sample} shows a non-uniform distribution of rays at the source PS. Such distribution is obtained from the triangulation refinement explained in the previous section. The procedure takes into account of the path followed by every rays traced. This allows to trace more rays close to the boundaries of the regions in PS, each of them is formed by all the rays that follow the same path when they propagate within the two-faceted cup. PS ray tracing is based on this initial rays distribution at the source. The target intensity in PS is calculated using only the rays that are located on the boundaries of those regions. Thus, in order to compute to obtain the profile of the intensity at the target, the boundaries need to be determined. In the next chapter we provide to different approach to find the boundaries $\partial$\set{R}{}{}$(\Pi)$ of the regions with positive luminance at the target PS \set{T}{}{}.
\begin{figure}[h]
