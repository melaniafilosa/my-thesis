...
Therefore, given an optical system with a line segment source $\point{S} = [-\variabile{a}, \variabile{a}]$ (labeled with $1$), the \'{e}tendue at the source coincides with the area of the entire PS \set{S}{}{}, given by:
...
To calculate the previous integral the $\alpha$-shapes method is applied to every region
...
Note that for systems where all the rays that leave \point{S} reach \point{T}, $U_1\big(\mbox{\set{R}{$1$}{}}(\Pi)\big) = U_1$ and it can be determined exactly by using Eq. \ref{eq:etenduesource}. Below we provide an example of the determination of the value of $\alpha$ for such kinds of systems.
