...
There are different ways to implement the ray tracing procedure. The efficiency of the ray tracing can related with the distribution of the rays at the source. 
Therefore, we would like to understand how the 
If the initial position and direction of the rays are chosen randomly we have Monte Carlo (MC) ray tracing which is explained in detail in the next paragraph. 
...
The concept of low discrepancy and the QMC method will be provided in last paragraph of this chapter. 
...
\begin{equation}
Var[S_\const{N}(f)] = \variabile{E}[(S_{\const{N}}(f)-\variabile{E}[S_{\const{N}}(f)])^2]= \variabile{E}[(S_{\const{N}}(f)-\variabile{E}[f])^2] = \sigma^2/\const{N}
\end{equation}
...
In particular, given a sequence of points $\{\variabile{y}_\variabile{n}\}_{\variabile{n} = 1, \cdots, \const{N}}$ in the interval $[a, b]^d\subseteq[0,1)^d$ and a function 
$f:[a,b]^d\mapsto \mathbb{R}$, QMC method approximates the integral of function $f$ on $[a,b]$ as the average of the values of $f$ on those points, that is:
\begin{equation}
\int_{[a,b]^d}f(y)\textrm{d}y \approx \frac{1}{\const{N}}\sum_{\variabile{i}=1}^{\const{N}}f(\variabile{y}_{\variabile{i}}).
\end{equation}
The idea is to generate the set of points in $[a,b]$ such that they are not random distributed but also not exactly uniformly distributed. 
%To this purpose, the concept of sequences uniformly distributed modulo one has been introduced, they are defined in the following.
%\begin{definition}
%An infinite sequence $\{\variabile{y}_\variabile{n}\}_{\variabile{n}\in\mathbb{N}_0}$ is said to be \textit{uniformly distributed modulo one} (or equidistributed), if for every interval $[a,b)\subseteq[0,1)^d$ it holds
%\begin{equation}
%\lim_{\const{N}\rightarrow \infty}\frac{\textrm{card}(A([a,b), \const{N}))}{\const{N}} = \lambda_d([a,b))
%\end{equation}
%where $\textrm{card}(A([a,b), \const{N}))$ is the cardinality of
%\begin{equation}
%A([a,b), N) = \{\variabile{n}\in \mathbb{N}_0: 0\leq\variabile{n}\leq \const{N}-1 \mbox{ and } \variabile{y}_{\variabile{n}}\in [a,b)\}\,,
%\end{equation}
%and $\lambda_d([a,b)) = \prod_{\variabile{j}=1}^{d}(b_\variabile{j}-a_\variabile{j})$ is the $d$-dimensional Lesbegue measure of the interval $[a,b)$
%\end{definition}
...
Usually, discrepancy is restricted to intervals $[a, b)\subseteq [0.1)^d$. Intuitively, it measures how much the samples differ from a uniform distribution.
...
\begin{definition}
Given a set $\mathcal{S} = \{y_1, \cdots, y_\const{N}\}$ of $\const{N}$ points in $[0,1)^d$. The discrepancy $D_\const{N}({\mathcal{S}})$ of $\mathcal{S}$ is defined as
\begin{equation}
D_\const{N}({\mathcal{S}}) = \sup_{a, b\in[0,1)^d}\Big|\frac{1}{\const{N}}\sum_{\variabile{i} = 1}^{\const{N}}\chi_{[a,b)}(\variabile{y}_\variabile{i})-\lambda_d([a, b))\Big|
\end{equation}
where $\chi_{[a,b)}(\variabile{y}_\variabile{i})$ is the indicator function of $[a, b)\subseteq [0.1)^d$ and $\lambda_d([a,b)) = \prod_{\variabile{j}=1}^{d}(b_\variabile{j}-a_\variabile{j})$ is the $d$-dimensional Lesbegue measure of the interval $[a,b)$.
\end{definition}
Often, it is enough to consider the discrepancy in the intervals $[0,a)\subseteq[0,1)^d$, in that case we talk about star discrepancy.
 \begin{definition}
Let $\mathcal{S} = \{y_1, \cdots, y_\const{N}\}$ be a set of $\const{N}$ points in $[0,1)^d$. The  star discrepancy $D^*_\const{N}({\mathcal{S}})$ of $\mathcal{S}$ is defined as:
\begin{equation}
D^*_\const{N}({\mathcal{S}}) = \sup_{a\in[0,1)^d}\Big|\frac{1}{\const{N}}\sum_{\variabile{i} = 1}^{\const{N}}\chi_{[0,a)}(\variabile{y}_\variabile{i})-\lambda_d([0, a))\Big|
\end{equation}
...
$\{\variabile{y}_{\variabile{n}}\}_{\variabile{n}\in \{0, 1,\cdots, 8\}}$ in base $\variabile{b}=2$ is:
\begin{equation}
\big(\phi_2(n)(\variabile{n})\big)_{\variabile{n}\in \{0, 1,\cdots, 8\}} = \Bigg\{0, \frac{1}{2}, \frac{1}{4}, \frac{3}{4}, \frac{1}{8},\frac{5}{8}, \frac{3}{8}, \frac{7}{8}, \frac{1}{16}\Bigg\} \,.
\end{equation}
...
\\ \indent The ray tracing procedure is computed considering the rays at the source with position and direction coordinates corresponding to the coordinates of the points of a low-discrepancy sequence. 
Sobol' sequences are an example of the extended Van der Corput sequences in base $\variabile{b}=2$ to $\variabile{d}\geq2$. 


\cite{bratley1988algorithm}
