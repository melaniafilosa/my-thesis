\chapter{The inverse ray mapping: analytic approach}\label{chap:raymapping1}
...
with $\lineai\neq \lineak$, the boundaries \set{S}{\lineai}{} and \set{T}{\lineak}{} are determined as follows. Let $(\variabile{x}_{\lineai, \ell}, \variabile{z}_{\lineai, \ell})$ and $(\variabile{x}_{\lineai, \textrm{r}},\variabile{z}_{\lineai, \textrm{r}})$ be the coordinates of the points located at the left and the right extreme of line $\lineai$, respectively.
...
The procedure starts on \set{T}{$4$}{} with a given direction $\variabile{p} = \const{constant}$ and with the parallel rays corresponding to the end points $(\variabile{q}_{\ell}, \variabile{p}) = (-\variabile{b}, \variabile{p})$ and $(\variabile{q}_{\textrm{r}}, \variabile{p}) = (\variabile{b}, \variabile{p})$. 
...
they are emitted. Then, another interval of parallel rays along the corresponding direction in \set{T}{\lineai}{} has to be considered and the intersection points between the line $\dir{}{} = \dir{t,}{\lineai}$ and $\partial$\set{T}{\lineai,}{\lineaj} (with $\variabile{j}\neq4$) are calculated, where $\dir{t,}{\lineai}$ is the new direction of the rays traced back.
...
$\variabile{p} = \variabile{p}^{\variabile{h}}$ in \set{T}{$4$}{}, where for the first step we take $\lineaj=4$.
\begin{figure}
\begin{minipage}[]{.48\textwidth}
\centering
