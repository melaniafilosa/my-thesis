...
$(\ref{Pij})$ and $(\ref{Rj})$, respectively. This composition depends on the path $\Pi$ followed by the rays where we refer to a path as the sequence of lines that
 a ray hits during its propagation from \point{S} to \point{T}. We indicate with $\mapnumb{M}_{1,4}(\Pi)$
...
\begin{figure}[t]
  \begin{center}
  \includegraphics[scale=.4]{final_T.eps}
  \end{center}
  \caption{Target phase space for the two-faceted cup divided into $100$ bins.
...
\begin{figure}[h]
  \begin{center}
  \includegraphics[scale=.4]{intensity.eps}
  \end{center}
  \caption{Intensities for the two-faceted cup with three different approaches.
  The red line shows the exact  intensity.
  The blue line depicts the intensity computed with MC raytracing with $10^7$ rays.
...
\begin{figure}[h]
  \begin{center}
  \includegraphics[scale=.4]{error_MC_PS.eps}
  \end{center}
  \caption{Error between the approximated intensities and the reference intensity as a function of the CPU time (in seconds).
  The convergence of the ray tracing approach is depicted with the blue line.
...
\caption{Target phase space of the $20$-faceted cup.
...
\begin{figure}[h!]
\centering
\includegraphics[scale=.4]{intensities10cup.eps}
...
\begin{figure}[h!]
\centering
\includegraphics[scale=.4]{error_10cup1.eps}
\caption{Error of the approximated intensities as a function of the CPU time. The convergence
of MC ray tracing is depicted with the blue line.
The PS error is shown with the green dots. The ray mapping method is more accurate than MC ray tracing and it is faster in case an error smaller than, say $10^{-4}$, is desired.}
